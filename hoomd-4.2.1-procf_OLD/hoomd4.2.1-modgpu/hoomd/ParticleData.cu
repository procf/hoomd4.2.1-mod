...
__global__ void gpu_scatter_particle_data_kernel(const unsigned int nwork,
    ...
    if (remove)
        ...
        for (unsigned int j = 0; j < 5; ++j)    
            p.net_virial_ind[j] = d_net_virial_ind[j * net_virial_ind_pitch + idx];
        //~
        p.tag = d_tag[idx];
        d_out[scan_remove] = p;
        ...
        //~ add virial_ind [RHEOINF]
        for (unsigned int j = 0; j < 5; ++j)
            d_net_virial_ind_alt[j * net_virial_ind_pitch + scan_keep]
                = d_net_virial_ind[j * net_virial_ind_pitch + idx];
        //~
...
/*! \param N Number of local particles
    ...
    \param d_net_virial_ind Net virial ~[RHEOINF]
    \param net_virial_ind_pitch Pitch of net virial array ~[RHEOINF]
    ...
    \param d_net_virial_ind Net virial (output) ~[RHEOINF]
...
unsigned int gpu_pdata_remove(const unsigned int N,
    ...
    assert(d_net_torque);
    assert(d_net_virial);
    assert(d_net_virial_ind); //~ [RHEOINF]
    assert(d_tag);
    assert(d_rtag);
    assert(d_pos_alt);
    assert(d_vel_alt);
    ...
    assert(d_net_virial_ind_alt); //~ [RHEOINF]
    assert(d_tag_alt);
    assert(d_out);
    assert(d_comm_flags);
...
__global__ void gpu_pdata_add_particles_kernel(unsigned int old_nparticles,
    ...
    //~ add virial_ind [RHEOINF] 
    for (unsigned int j = 0; j < 5; ++j)
        d_net_virial_ind[j * net_virial_ind_pitch + add_idx] = p.net_virial_ind[j];
    //~
    d_tag[add_idx] = p.tag;
